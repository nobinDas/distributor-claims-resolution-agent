...
Generates realistic synthetic data for the UNFI Supplier Deduction & Claims
Resolution Agent capstone project, and loads it into the BigQuery tables
created by create_bigquery_tables.py.
...
Populates, in dependency order:
    ...
    receiving         (POD)
    invoices          (EDI 810)
    deduction_claims  (EDI 812)

Includes THREE deliberately planted demo cases (clearly marked below) so you
always have a known-good story for your capstone demo, on top of a larger
batch of randomly generated background data for volume/realism.

Prerequisites:
    pip install google-cloud-bigquery pandas faker

Usage:
    python generate_synthetic_data.py --project YOUR_PROJECT_ID
    python generate_synthetic_data.py --project YOUR_PROJECT_ID --dataset unfi_deductions --seed 42
"""
...
# ---------------------------------------------------------------------------
# Background (random) data generation
# ---------------------------------------------------------------------------
...
def generate_shipments_and_receiving(pos_df):
    """
    For each PO, generate one shipment (ASN) and one receiving (POD) record.
    Most match the PO exactly. A random subset gets a genuine shortage,
    simulating real-world variance, independent of the specific planted
    demo cases (which are added separately with fixed, known-good IDs).
    """
    shipments, receiving = [], []
    ...
    for _, po in pos_df.iterrows():
        ...
        # ~12% of background shipments have a minor, real shipping shortfall
        ...
        # ~8% of background receipts show a further discrepancy at the dock
...
def generate_invoices(pos_df):
    ...
    for _, po in pos_df.iterrows():
        ...
        # Most invoices bill exactly what was ordered; a small % over-bill
...
def generate_deduction_claims(pos_df, invoices_df, receiving_df, claim_rate=0.30):
    """
    Generates background deduction claims for a random subset of invoices,
    mixing valid, invalid, and ambiguous cases so the agent has a realistic
    volume of cases to work through beyond the three named demo cases.
    """
    rows = []
    claim_counter = 1
    ...
    for _, invoice in invoices_sample.iterrows():
        ...
        if shortfall > 0:
            claim_amount = round(shortfall * 5.0, 2)  # simplified valuation
            dispute_text = (
...
# PLANTED DEMO CASES
#
# These three cases are deliberately constructed so your capstone demo has a
# reliable, explainable "before / after" story. Each one exercises a
# different branch of the agent's logic:
#
#   DEMO-001  Genuine, clearly VALID shortage claim
#             -> agent should confirm the deduction and close it fast.
#
#   DEMO-002  Clearly INVALID deduction (goods fully received, no discrepancy)
#             -> agent should flag it for reversal/credit back to supplier.
#
#   DEMO-003  AMBIGUOUS case (damaged-goods note, partial/unclear match)
#             -> agent should decline to auto-resolve and route to a human.
# ---------------------------------------------------------------------------
...
def load_df(client, dataset_id, table_name, df):
    if df.empty:
    ...
    job.result()
    print(f"Loaded {len(df):>4} rows into {table_ref}")


def main():
    parser = argparse.ArgumentParser(description="Generate and load synthetic data for the UNFI Claims Resolution Agent")
    parser.add_argument("--project", required=True)
    parser.add_argument("--dataset", default="unfi_deductions")
    ...
    load_df(client, dataset_id, "suppliers", suppliers_df)
    load_df(client, dataset_id, "deduction_codes", codes_df)
    load_df(client, dataset_id, "purchase_orders", pos_df)
    load_df(client, dataset_id, "shipments", shipments_df)
    load_df(client, dataset_id, "receiving", receiving_df)
    load_df(client, dataset_id, "invoices", invoices_df)
    load_df(client, dataset_id, "deduction_claims", claims_df)
